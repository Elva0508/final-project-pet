...
export const MissionDetailSticky = ({ mission_id, user_id }) => {
    ...
    <>
      <section className="ask-and-apply d-flex justify-content-center align-items-center">
        <div className="position-absolute fav" onClick={toggleFavorite}>
          {isFavorite ? (
            <div className="d-flex flex-column justify-content-end align-items-cnter">
        ...
        <button
          className="ask-and-apply-btn btn-outline-confirm d-flex align-items-center justify-content-center"
          onClick={() => {
            router.push("/product/" + user_id);
          }}
        >
          <PiWechatLogoThin />
          線上詢問
        ...
        <button
          className="ask-and-apply-btn btn-second d-flex align-items-center justify-content-center"
...
export default function MissionDetail() {
  ...
  const getMissionDetail = async (mission_id) => {
    // 接受 mission_id 作為參數
    try {
      const response = await axios.get(
        `http://localhost:3005/api/mission/mission-details/${mission_id}`
      );
  ...
  useEffect(() => {
    if (mission_id) {
      ...
      axios
        .get(
          `http://localhost:3005/api/mission/mission-details-img/${mission_id}`
        )
        .then((response) => {
      ...
      axios
        .get(
          `http://localhost:3005/api/mission/mission-details-map/${mission_id}`
        )
        .then((response) => {
          ...
          console.log(
            "lat是:" +
              response.data.data.location.lat +
              "lng是:" +
              response.data.data.location.lng
          );
        })
        .catch((error) => {
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:3005/api/mission/add-record",
        requestData
      );
  ...
  return (
    <>
      {/* Modal */}
      <div
        className={`modal fade apply-modal`}
        id="exampleModal"
        tabindex="-1"
        ...
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title size-4" id="exampleModalLabel">
                立即應徵
              </h5>
              <button
                type="button"
            ...
            <div className="modal-body">
              <div className="profile d-flex justify-content-center align-items-center">
                ...
                <div className="justify-content-center">
                  <div className="size-4">雅晴</div>
                  <p className="size-6 mt-1">25歲</p>
              ...
              <div className="recommend mt-4">
                ...
                <div className="auto-send d-flex my-4 align-items-center">
                  ...
                  <div className="size-6 ms-2">
                    自動發送小幫手履歷
                    <span className="size-7">（需開啟小幫手資料）</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="container mission-detail my-3">
            ...
            <header className="mt-3 p-4">
              ...
              <p className="size-6 mt-2">
                最後更新：{formatDate(v.update_date)}
              </p>
            </header>
            <section className="description my-4 py-1 ">
              <div className="item d-flex flex-column flex-sm-row ">
                <div className="item-title size-5">預算金額：</div>
                <p className="size-6 d-flex align-items-center ms-4 ms-sm-0 salary">
                  NT$ {v.price} / 次
                </p>
              </div>
              <div className="item d-flex flex-column flex-sm-row">
                <div className="item-title size-5">任務日期：</div>
              ...
              <div className="item d-flex flex-column flex-sm-row">
                <div className="item-title size-5">任務地點：</div>
                <p className="size-6 d-flex align-items-center ms-4 ms-sm-0">
                  {v.city}
                  {v.area}
                  {v.location_detail}
              ...
              <div className="d-flex justify-content-center">
                <MapComponent
                  key={`map-${missionLocation.lat}-${missionLocation.lng}`}
                  lat={missionLocation.lat}
                  lng={missionLocation.lng}
                />
              </div>
              ...
              <div className="item d-flex flex-column flex-sm-row">
                <div className="item-title size-5">任務類型：</div>
                <p className="size-6 d-flex align-items-center ms-4 ms-sm-0">
                  {" "}
                  {(() => {
              ...
              <div className="item d-flex flex-column flex-sm-row">
                <div className="item-title size-5">支付方式：</div>
                <p className="size-6 d-flex align-items-center ms-4 ms-sm-0">
                  {(() => {
            ...
            <div className="d-none">
              <MissionDetailSticky mission_id={mission_id} />
            </div>
          </div>
        );
      })}
