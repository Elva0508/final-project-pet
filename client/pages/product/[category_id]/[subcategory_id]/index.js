...
export default function ProductList() {
      ...
      const fetchData = async () => {
        ...
        try {
          const response = await axios.get(
            `http://localhost:3005/api/product/product/subcategory/${subcategory_id}`
          );
          const data = response.data.result;
          console.log(data);
          setProductData(data);
  ...
  useEffect(() => {
    axios
      .get("http://localhost:3005/api/product/subcategory")
      .then((response) => {
  ...
  const handleSortChange = (event) => {
    ...
    if (selectedValue == "price_desc") {
      const newProduct = productData.sort(
        (a, b) => b.specialoffer - a.specialoffer
      );
      setProductData(newProduct);
    } else if (selectedValue == "price_asc") {
      const newProduct = productData.sort(
        (a, b) => a.specialoffer - b.specialoffer
      );
      setProductData(newProduct);
    } else {
      const newProduct = productData.sort(
  ...
  const handleSearch = (search) => {
    ...
    axios
      .get("http://localhost:3005/api/product/filter_sort", {
        params: {
          search,
        },
      })
  ...
  const handlePriceVendorfilter = (vendor, minPrice, maxPrice) => {
    console.log(
      "handlePriceVendorfilter 函数被使用，search結果:",
      vendor,
      minPrice,
      maxPrice
    );
    ...
    if (selectedSort === "" || selectedSort == null) {
      ...
      const newProduct = finalData.sort(
        (a, b) => b.specialoffer - a.specialoffer
      );
      setProductData(newProduct);
      ...
      <div className="product-list">
        <div className="container">
          ...
          <div className="product-offcanvas-m d-block d-lg-none ">
            <button
              className="product-sidebar-btn btn-confirm size-5"
              type="button"
                  ...
                  {subcategoryData.result.map((category, index) => (
                    <div className="accordion-item" key={index}>
                      <h2
                        ...
                        id={`panelsStayOpen-headingCategory-${index}`}
                      >
                        <button
                      ...
                      <div
                        id={`panelsStayOpen-collapseCategory-${index}`}
                            ...
                            if (v.category_id === category.category_id) {
                              return (
                                <button
                                  ...
                                  onClick={() => {
                                    router.push(
                                      `/product/${category.category_id}/${v.subcategory_id}`
                                    );
                  ...
                  <div className="card filter-card">
                    ...
                    <div className="card-body">
                      ...
                      <div className="col-12">
                        <label for="inputprice size-6" className="form-label">
                          價格區間
                        </label>
                      ...
                      <div className="col-12 mt-2">
                        <label for="brand size-6" className="form-label">
                          品牌
                        </label>
